...
<script>
	...
	layui.use('table', function(){
		...
		dicTabel =table.render({
			...
			,limit:5
			,limits:[5]
			,url:'/StopCard/Devic.action'
			// ,method:'post'
			,page: true
			,id:'testReload'
			,cols: [[ //标题栏
				{field: 'devicid', title: '自助设备序号', sort: true ,align:'center'}
				,{field: 'devicearea', title: '自助设备放置区域',align:'center' }
				,{field: 'devicname', title: '自助设备名字',align:'center'}
				,{field: 'devictime', title: '设备添加时间',align:'center' }
				,{field: 'statename', title: '自助设备状态',align:'center' }
				...
				layer.open({
					...
					,yes: function(index, layero){
                       ...
                       alert("设备区域-----"+devicearea);
						...
						$.ajax({
							type: "POST",//提交的方式
							url: "/StopCard/adddevic.action",//提交的地址
							data: ob,//提交的数据
		...
		table.on('tool(test)', function(obj){
			...
			if(obj.event === 'detail'){
				...
				layer.confirm('真的删除行么', function(index){
					...
					$.ajax({
						type: "POST",//提交的方式
						url: "/StopCard/deletedevic.action",//提交的地址
						data: ob,//提交的数据
						dataType: "json",//希望返回的数据类型
						async: true,//异步操作
						success: function (msg) {//成功的方法  msg为返回数据
