...
<%--		</div>--%>
		<div class="layui-form">
					...
					<c:forEach items="${requestScope.tableBody}" var="i" begin="0" step="1">
						<tr>
							...
							<td>
								...
								<c:if test="${j.arrangetime==requestScope.tableHead[0]}">
							...
							<td>
								...
								<c:forEach items="${i.value}" begin="0" step="1" var="j">
									<c:if test="${j.arrangetime==requestScope.tableHead[1]}">
										<c:choose>
											<c:when test="${j.workerid == 26}">
												<button type="button" class="btn btn-schedule layui-btn layui-btn-primary" value="${j.workerid}"
												        onclick="showRegistered(this)"couId="${j.arrangeid}" >${j.workeraccount}</button>
							...
							<td>
								...
								<c:forEach items="${i.value}" begin="0" step="1" var="j">
									<c:if test="${j.arrangetime==requestScope.tableHead[2]}">
										<c:choose>
											<c:when test="${j.workerid == 26}">
												<button type="button"  class="btn btn-schedule layui-btn layui-btn-primary" value="${j.workerid}"
												        onclick="showRegistered(this)" couId="${j.arrangeid}">${j.workeraccount}</button>
							...
							<td>
								...
								<c:forEach items="${i.value}" begin="0" step="1" var="j">
									<c:if test="${j.arrangetime==requestScope.tableHead[3]}">
										<c:choose>
											<c:when test="${j.workerid == 26}">
												<button type="button" class="btn btn-schedule layui-btn layui-btn-primary" value="${j.workerid}"
												        onclick="showRegistered(this)"couId="${j.arrangeid}" >${j.workeraccount}</button>
							...
							<td>
								...
								<c:forEach items="${i.value}" begin="0" step="1" var="j">
									<c:if test="${j.arrangetime==requestScope.tableHead[4]}">
										<c:choose>
											<c:when test="${j.workerid == 26}">
												<button type="button" class="btn btn-schedule layui-btn layui-btn-primary" value="${j.workerid}"
												        onclick="showRegistered(this)" couId="${j.arrangeid}">${j.workeraccount}</button>
			...
			<a href="/StopCard/weekcourseTable.action?now-Date=${requestScope.tableHead[0]}&doWhich=上一周&">
				<input type="button" class="btn btn-default layui-btn" value="上一周" onclick="weeks(this)"/>
			</a>

			<a href="/StopCard/weekcourseTable.action?now-Date=${requestScope.tableHead[0]}&&doWhich=下一周&">
...
<script>
	...
	function showRegistered(node) {
		layer.open({
			...
			, title: '配置课程'
			, offset: 'auto'
			, content: '/StopCard/findSubject.action'
			, area: ['450px', '350px']
			, btn: ['确定', '取消']
			, shade: 0
			,  yes: function (index, layero) {
               ...
               alert("subjects----------"+subjects);
				$.ajax({
					type: "POST",//提交的方式
					url: "/StopCard/ScheduleGai.action",
					data: ob,//提交的数据
